...
class ActorRadar(pyglet.window.Window):
    def __init__(self):
        super().__init__(WINDOW_SIZE, WINDOW_SIZE, caption="Teste de Posições dos Atores")
        ...
        pyglet.clock.schedule_interval(self.update_actors, 0.5)
    
    def update_actors(self, dt):
        """Atualiza as posições dos atores"""
        try:
            # Limpar atores antigos
            self.actors.clear()
            ...
            self.info_label.text = f"Atores: {actor_count} | Posição: {local_pos[0]:.1f}, {local_pos[1]:.1f}, {local_pos[2]:.1f} | Yaw: {yaw:.1f}°"
            ...
            for i in range(min(actor_count, 500)):
                ...
                try:
                    ...
                    # Criar marcador
                    marker = shapes.Circle(screen_x, screen_y, 4, color=color, batch=self.batch)
                    ...
                    self.actors.extend([marker, label])
                except:
                    continue
            
            # Atualizar contador
            self.info_label.text += f" | Jogadores: {players_count} | NPCs: {npcs_count}"
