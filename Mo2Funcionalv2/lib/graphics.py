...
class Radar(RadarObject):
    ...
    def __init__(self):
        ...
        # FOV removido conforme solicitado

        self.build_rings()
        self.compass = Compass()

        # red line indicating North direction
        facing = shapes.Line(
            x=Radar.CENTER[0],
            y=Radar.CENTER[1],
            x2=Radar.CENTER[0],
...
class Compass(RadarObject):
    __slots__ = "directions", "north", "cardinal_labels"
    ...
    def __init__(self):
        ...
        # Adicionar rótulos para os pontos cardeais
        self.cardinal_labels = {}
        
        # Posicionar os rótulos dos pontos cardeais na borda do radar
        # O "N" fica alinhado com a seta vermelha (na parte superior)
        label_distance = Radar.RADIUS - 15
        diagonal_distance = (label_distance * 0.7)  # Aproximadamente cos(45°) * distance
        
        # Posicionar os pontos cardeais fixos na borda do radar
        self.cardinal_labels["N"] = pyglet.text.Label(
            "N", font_name="DejaVu Sans", font_size=12,
            x=Radar.CENTER[0], y=Radar.CENTER[1] + label_distance,
            anchor_x="center", anchor_y="center",
            color=(255, 255, 255, 255),  # Vermelho para destacar
            batch=Radar.BATCH, group=highest
        )
        ...
        for direction, pos in cardinal_points.items():
            self.cardinal_labels[direction] = pyglet.text.Label(
                direction,
                font_name="DejaVu Sans",
                font_size=12,
                x=Radar.CENTER[0] + pos[0],
                y=Radar.CENTER[1] + pos[1],
                anchor_x="center",
                anchor_y="center",
                color=(255, 255, 255, 255),
                batch=Radar.BATCH,
...
class PlayerBlip(GameObject):
    """player icon that shows as a red circle with HP"""

    RADIUS = 4  # Tamanho reduzido
    # name+guild, hp, elevation
    FONT_SIZES = (10, 8, 7)
    ...
    def __init__(self, actor):
        ...
        # Círculo vermelho para jogadores
        self.marker = shapes.Circle(
            x=0,
            y=0,
    ...
    def update(self, your_pos: tuple[float, float, float], view_matrix):
        ...
        # Posicionar o texto acima do marcador
        self.label.x = screen_x
        self.label.y = screen_y + PlayerBlip.RADIUS + 5

        elevation = self.actor.pos[2] - your_pos[2]
        alpha = self.get_alpha(elevation)
        
        # Atualizar a cor com a transparência baseada na elevação
        self.marker.color = (255, 0, 0, alpha)  # Vermelho com transparência

        readable_elevation = self.elevation_string(elevation)
        
        # Adicionar nome da guilda e HP ao texto do jogador
        try:
            hp, max_hp = self.actor.health
            
            # Verificar se o jogador tem uma guilda
            if hasattr(self.actor, "guild_name") and self.actor.guild_name:
                # Adicionar nome da guilda entre colchetes
                new_label = f"{self.actor.name} [{self.actor.guild_name}]\n({round(hp)}/{round(max_hp)}){readable_elevation}"
            else:
                new_label = f"{self.actor.name}\n({round(hp)}/{round(max_hp)}){readable_elevation}"
        except (ValueError, TypeError):
            # Mesmo sem HP, mostrar a guilda se disponível
            if hasattr(self.actor, "guild_name") and self.actor.guild_name:
                new_label = f"{self.actor.name} [{self.actor.guild_name}]{readable_elevation}"
            else:
